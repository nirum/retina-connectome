...
loadCell = function() {
   ...
   // get cell ID
   var cellID = parseInt(document.forms['cellform']['cellID'].value);
   ...
   d3.json('../json/cell' + cellID + '.json', function(error, data) {
      ...
      // get extent of data
      var x_extent = d3.extent(data.nodes, function(d) {
         return d[xIdx]
      });
      var y_extent = d3.extent(data.nodes, function(d) {
         return d[yIdx]
      });
      ...
      // add nodes to svg
      svg.selectAll("circle").data(data.nodes).enter().append("circle").attr("cx", function(d) {
         return x(d[xIdx]);
      }).attr("cy", function(d) {
         return y(d[yIdx]);
      }).attr("r", rad);
      ...
      d3.select("#loading").remove();
			d3.select("#title")[0][0].textContent = "Cell " + cellID;
